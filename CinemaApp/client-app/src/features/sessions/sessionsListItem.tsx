    ...
    return (
        <Box display="flex" flexDirection="row" sx={{
            ...
            <Box display="flex" justifyContent="center" alignItems="center" flexDirection="column" gap="5px">
                <Button variant="contained" color="secondary">Buy ticket</Button>
                {userStore.user?.role === "admin" && (
                    <Box display="flex" flexDirection="row" gap="20px">
                        <Button variant="contained" color="warning">Edit</Button>
                        <Button variant="contained" onClick={() => { cinemaStore.deleteSession(session.id)}} color="error">Delete</Button>  
                    </Box>
                )}
            </Box>
        </Box>
